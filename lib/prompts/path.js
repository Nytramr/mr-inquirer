...
Prompt.prototype.onKeypress = function(input) {
  ...
  this.rl.output.unmute();
  this.clean(this.optionsPaths.length).render();
  var path = this.rl.line;
  ...
  var message = path + this.optionsPaths.map(function (e){
    return "\n" + input.key.name; 
  });
  var w = this.write(message);//.restoreCursorPos();
  console.log(w);
  this.rl.setPrompt(path);
  this.rl.output.mute();
};
...
////  }
//
//  this.opt.choices.setRender( pathRender );
//
...
// */
//
//function pathRender( pointer, newRoute ) {
//  var output = "";
//  
