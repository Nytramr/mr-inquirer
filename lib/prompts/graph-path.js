...
Prompt.prototype._run = function( cb ) {
  ...
  this.list = pathUtils.getDirContenList(this.currentDir, this.opt.dirOnly);
...
Prompt.prototype.onSubmit = function( input ) {
  ...
  if ( this.selected ) {
    ...
    if ( this.optionSelected >= 0 ) {
      this.selected = pathUtils.join(this.currentDir, this.filteredList[this.optionSelected]);
    }
...
Prompt.prototype.update = function ( input ) {
  ...
  if ( input.key && input.key.name === "up" ) {
    ...
    if (currentDir !== this.currentDir) {
      this.list = pathUtils.getDirContenList(currentDir, this.opt.dirOnly);
